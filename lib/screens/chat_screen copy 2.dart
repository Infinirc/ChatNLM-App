...
Future<void> _loadInitialData() async {
  ...
  try {
    ...
    if (!authProvider.isTrialMode && authProvider.isAuthenticated) {
      ...
      if (conversationProvider.conversations.isEmpty) {
        ...
        final latestConversation = conversationProvider.conversations.first;
        await conversationProvider.setCurrentConversation(latestConversation);
        await chatProvider.loadConversationMessages(latestConversation.id);

        // Add a small delay to ensure states are updated
        await Future.delayed(const Duration(milliseconds: 100));

        // Check if title needs to be generated
        if (mounted && conversationProvider.currentConversation != null) {
          final currentConversation = conversationProvider.currentConversation!;
          final messages = chatProvider.messages;
          
          // Check for default title and existing messages
          if (currentConversation.title == 'New Chat' && messages.length >= 2) {
            // Find the latest user and AI messages
            final userMessage = messages.lastWhere((msg) => msg.isUser);
            final aiMessage = messages.firstWhere((msg) => !msg.isUser);

            // Generate title if both messages have content
            if (userMessage.content.isNotEmpty && aiMessage.content.isNotEmpty) {
